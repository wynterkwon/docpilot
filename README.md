...
- **HWPX 스타일 인식 생성** — 플레이스홀더 위치의 폰트 크기·정렬·표 셀 너비를 자동 분석해 LLM에 전달, 서식에 어울리는 내용 생성
- **템플릿 자동 생성** — 샘플 HWPX 문서에서 공통 섹션 구조 추출 (샘플 스타일 자동 상속)
...
HWPX 템플릿을 사용하면 `{{플레이스홀더}}`가 위치한 문단의 서식 정보를 자동으로 분석해 LLM 프롬프트에 포함합니다.

| 추출 정보 | 설명 | LLM에 전달되는 형태 |
|-----------|------|---------------------|
| 폰트 크기 | `charPr.height` → pt 변환 | `10pt 바탕` |
| 정렬 | `paraPr.align` | `JUSTIFY`, `CENTER` 등 |
| 표 셀 너비 | `hp:cellSz.width` → mm 변환 | `표 셀 너비 120mm` |
| 볼드/이탤릭 | `hh:bold`, `hh:italic` 요소 존재 여부 | `볼드`, `이탤릭` |
...
- "본문": [스타일: 10pt 바탕, JUSTIFY, 표 셀 너비 120mm]
- "결론": [스타일: 10pt 바탕, JUSTIFY]
```
